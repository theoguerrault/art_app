...
<section class="panel">
    ...
    <div class="action-buttons">
      ...
      {:else}
        {#if artwork.image_url_full}
          <Button variant="secondary" size="sm" onclick={verifyImage} loading={verifyingImage}>🤖 Vérifier avec l'IA</Button>
        {/if}
  ...
  <div class="image-section">
    <div class="current-image">
      {#if artwork.image_url_full}
        <img src={artwork.image_url_full} alt={(artwork.artwork_translations?.[0]?.title || '')} class="preview-image" />
      {:else}
    ...
    {#if verificationResult}
      <div class="verification-result" class:valid={verificationResult.isValid} class:invalid={!verificationResult.isValid}>
        <h4>{verificationResult.isValid ? '✅ Image validée par Gemini' : '❌ Incohérence détectée'}</h4>
...
<style>
  ...
  .verification-result.valid {
    background-color: color-mix(in oklch, oklch(0.6 0.15 140) 15%, transparent);
    border: 1px solid oklch(0.6 0.15 140);
  }

  .verification-result.invalid {
    background-color: color-mix(in oklch, oklch(0.6 0.2 25) 15%, transparent);
