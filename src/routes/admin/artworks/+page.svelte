...
<div class="admin-view">
  <header class="admin-header sticky-header">
    ...
    <form class="search-bar" action="/admin/artworks" method="GET" data-sveltekit-keepfocus data-sveltekit-replacestate>
  ...
  <div class="grid-catalog-minimal">
    {#each data.artworks as artwork (artwork.id || artwork)}
      <a data-sveltekit-preload-data="hover" href={`/admin/artworks/${artwork.id}`} class="artwork-card-minimal">
        <div class="thumb-wrapper">
          <img src={artwork.image_url_thumb} alt={(artwork.artwork_translations?.[0]?.title || '')} loading="lazy" decoding="async" />
          
          <div class="status-indicator">
            {#if artwork.artwork_translations[0]}
              {#if artwork.artwork_translations[0].verification_status === 'VERIFIED'}
                <div class="badge verified" title="Vérifié">
                  <CheckCircle size={14} weight="fill" />
                  <span>Vérifié</span>
                </div>
              {:else if artwork.artwork_translations[0].verification_status === 'PENDING' || artwork.artwork_translations[0].verification_status === 'PENDING_VALIDATION'}
              ...
              {:else}
                <div class="badge unknown">
                  <span>{artwork.artwork_translations[0].verification_status}</span>
                </div>
              {/if}
            {:else}
        ...
        <div class="art-info">
          <h3 class="art-title">{(artwork.artwork_translations?.[0]?.title || '')}</h3>
          <p class="art-artist">{artwork.artists?.artist_translations?.[0]?.name}</p>
          
          {#if !artwork.artwork_translations[0] || artwork.artwork_translations[0].verification_status !== 'VERIFIED'}
