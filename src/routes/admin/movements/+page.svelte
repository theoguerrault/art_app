...
<div class="admin-view">
  <header class="admin-header sticky-header">
    <h1 class="page-title">Administration des Mouvements Artistiques</h1>
    
    <form class="search-bar" action="/admin/movements" method="GET" data-sveltekit-keepfocus data-sveltekit-replacestate>
  ...
  <div class="art-grid">
    {#each data.movements as movement (movement.id)}
      <a data-sveltekit-preload-data="hover" href={`/admin/movements/${movement.id}`} class="art-card">
        <div class="art-status-wrapper">
          <div class="art-status">
            {#if movement.movement_translations[0]}
              {#if movement.movement_translations[0].verification_status === 'VERIFIED'}
                <div class="badge verified" title="Contenu validé">
                  <CheckCircle size={16} weight="fill" />
                  <span>Validé</span>
                </div>
              {:else if movement.movement_translations[0].verification_status === 'PENDING'}
              ...
              {:else}
                <div class="badge unknown">
                  <span>{movement.movement_translations[0].verification_status}</span>
                </div>
              {/if}
            {:else}
        ...
        <div class="art-info">
          <h3 class="art-title">{(movement.movement_translations?.[0]?.name || '')}</h3>
          
          {#if !movement.movement_translations[0] || movement.movement_translations[0].verification_status !== 'VERIFIED'}
