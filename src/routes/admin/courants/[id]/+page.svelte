<script lang="ts">
  ...
  let content = $derived(data.courant.courant_translations[0]);
  ...
  async function saveEdit() {
    try {
      const res = await apiClient.post(`/api/admin/courants/${courant.id}/edit-description`, { content: editContent });
      if (res.ok) {
  ...
  async function generateContent() {
    ...
    generating = true;
    try {
      const res = await apiClient.post(`/api/admin/courants/${courant.id}/generate`);
      if (res.ok) {
        const json = await res.json();
        await invalidateAll();
      } else {
  ...
  async function validateManual() {
    validating = true;
    try {
      const res = await apiClient.post(`/api/admin/courants/${courant.id}/validate`);
      if (res.ok) {
        const json = await res.json();
        await invalidateAll();
      } else {
...
<div class="admin-detail-view">
  <div class="top-nav">
    <a data-sveltekit-preload-data="hover" href="/admin/courants" data-sveltekit-prefetch class="back-link">
      <ArrowLeft size={20} />
