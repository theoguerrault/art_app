...
<div class="admin-view">
  <header class="admin-header sticky-header">
    ...
    <form class="search-bar" action="/admin/courants" method="GET" data-sveltekit-keepfocus data-sveltekit-replacestate>
  ...
  <div class="art-grid">
    {#each data.courants as courant (courant.id)}
      <a href={`/admin/courants/${courant.id}`} class="art-card">
        <div class="art-status-wrapper">
          <div class="art-status">
            {#if courant.courant_translations[0]}
              {#if courant.courant_translations[0].verification_status === 'VERIFIED'}
                <div class="badge verified" title="Contenu validé">
                  <CheckCircle size={16} weight="fill" />
                  <span>Validé</span>
                </div>
              {:else if courant.courant_translations[0].verification_status === 'PENDING'}
              ...
              {:else}
                <div class="badge unknown">
                  <span>{courant.courant_translations[0].verification_status}</span>
                </div>
              {/if}
            {:else}
        ...
        <div class="art-info">
          <h3 class="art-title">{(courant.courant_translations?.[0]?.nom || '')}</h3>
          
          {#if !courant.courant_translations[0] || courant.courant_translations[0].verification_status !== 'VERIFIED'}
